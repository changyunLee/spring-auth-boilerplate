...
public class AdminService {
    ...
    public void changeUserRole(Long id, AdminDto.RoleChangeRequest request) {
        ...
        if (user.getEmail().equals(currentEmail)) {
            throw new IllegalArgumentException("자기 자신의 권한을 변경할 수 없습니다.");
        }
    ...
    public AdminDto.SuspendResponse suspendUser(Long id, AdminDto.SuspendRequest request) {
        ...
        return new AdminDto.SuspendResponse(user.getId(), user.getEmail(), user.isSuspended(),
                user.getSuspendedReason(), user.getSuspendedUntil());
    }
    ...
    public List<AdminDto.SuspiciousIpResponse> getSuspiciousIps(int hours) {
        ...
        return data.stream()
                .map(row -> new AdminDto.SuspiciousIpResponse(
                        (String) row[0],
